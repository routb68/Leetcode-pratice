...
public:
	vector<vector<int>> kSmallestPairs(
		...
		while (k-- and pq.size()) {
			...
			if (i + 1 < nums1.size() and
				us.find(make_pair(i + 1, j)) == us.end()) {
				pq.push(
					make_pair(nums1[i + 1] + nums2[j], make_pair(i + 1, j)));
				us.insert(make_pair(i + 1, j));
			}
			if (j + 1 < nums2.size() and
				us.find(make_pair(i, j + 1)) == us.end()) {
				pq.push(
					make_pair(nums1[i] + nums2[j + 1], make_pair(i, j + 1)));
